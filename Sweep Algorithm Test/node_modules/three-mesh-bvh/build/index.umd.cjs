(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.MeshBVHLib = global.MeshBVHLib || {}, global.THREE, global.THREE));
	...
	class ObjectBVH extends threeMeshBvh.BVH {
			...
			this.shapecast( {
				...
				intersectsBounds: box => {
					...
					if ( firstHitOnly ) {
						...
						return raycaster.ray.origin.distanceTo( _vec ) < closestDistance ? threeMeshBvh.INTERSECTED : threeMeshBvh.NOT_INTERSECTED;

					} else {

						return _ray$1.intersectsBox( box ) ? threeMeshBvh.INTERSECTED : threeMeshBvh.NOT_INTERSECTED;
	...
	class SkinnedMeshBVH extends threeMeshBvh.GeometryBVH {
		...
		constructor( mesh, options = {} ) {
			...
			super( mesh.geometry, {
				...options,
				[ threeMeshBvh.SKIP_GENERATION ]: true,
			} );
			this.mesh = mesh;

			if ( ! options[ threeMeshBvh.SKIP_GENERATION ] ) {
		...
		shapecast( callbacks ) {
			...
			const triangle = new threeMeshBvh.ExtendedTriangle();
			return super.shapecast(
				{
					...callbacks,
					intersectsPrimitive: callbacks.intersectsTriangle,
		...
		raycastObject3D( object, raycaster, intersects = [] ) {
			...
			this.shapecast( {
				...
				intersectsBounds: box => {
					...
					return _ray.intersectsBox( box ) ? threeMeshBvh.INTERSECTED : threeMeshBvh.NOT_INTERSECTED;
